...
class IntegratedTracker:
    def __init__(self):
        ...
        self.emotion_detection_enabled = True  # <-- NEW: State variable to toggle emotion detection
    ...
    def start(self):
        ...
        print("- P: Toggle emotion detection") # <-- MODIFIED: Added control info
        print("- Q: Quit")
        print("")
    ...
    def _run_main_loop(self):
        while not self._stop_event.is_set():
            ...
            # --- Emotion Detection ---
            if self.emotion_detection_enabled: # <-- MODIFIED: Conditional execution
            ...
            if key == ord('q'):
                ...
                print(f"Sensitivity: {self.nose_tracker.sensitivity:.1f}")
            elif key == ord('p'): # <-- NEW: Key handler to toggle emotion detection
                ...
                if not self.emotion_detection_enabled:
                    self.latest_emotion_data = {"emotion": "Paused"}
    ...
    def _draw_combined_status(self, frame):
        # <-- MODIFIED: Entire block updated for new status line
        # Increased box height to fit the new line
        box_h = 220
        cv2.rectangle(frame, (10, 10), (640, box_h), (0, 0, 0), -1)
        cv2.rectangle(frame, (10, 10), (640, box_h), (255, 255, 255), 2)
        ...
        emotion_status = "ACTIVE" if self.emotion_detection_enabled else "PAUSED"
        ...
        cv2.putText(frame, f"Emotion Detection: {emotion_status}", (20, 110),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    (0, 255, 0) if emotion_status == 'ACTIVE' else (0, 0, 255), 2)

        # Data display
        cv2.putText(frame, f"Blinks: {self.blink_detector.blink_count}", (20, 135),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        cv2.putText(frame, f"Sensitivity: {self.nose_tracker.sensitivity:.1f}", (20, 160),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        current_emotion = self.latest_emotion_data.get("emotion", "N/A")
        cv2.putText(frame, f"Emotion: {current_emotion}", (20, 185),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        
        # Help text
        cv2.putText(frame, "Hold CTRL to move cursor | Release for blink click",
                    (20, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
